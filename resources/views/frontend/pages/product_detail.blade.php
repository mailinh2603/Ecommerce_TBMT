...
@section('main-content')
    ...
    <s>{{ number_format($product_detail->price, 0, ',', '.') }}₫</s>
											...
											<div class="product-buy">
												...
												<p class="cat mt-1">Loại sản phẩm  :<a href="{{route('product-sub-cat',[$product_detail->cat_info['slug'],$product_detail->sub_cat_info['slug']])}}">{{$product_detail->sub_cat_info['title']}}</a></p>
												@endif
...
<div class="rating-des">
	...
	<p><strong>Người phản hồi:</strong> {{ $response->user->name ?? 'N/A' }}</p><br>
        <div class="response mb-3 p-2 border rounded shadow-sm" style="max-width: 60%;margin-left: 20px; background-color: #f8f9fa; box-shadow: 0 4px 4px 0 rgba(0, 0, 0, 0.2), 0 4px 5px 0 rgba(0, 0, 0, 0.19);">
            ...
            <p>{{ $response->content ?? 'No content available' }}</p>
        </div><br>
    @empty
    ...
    @auth
    <form action="{{ route('response.store', $data->id) }}" method="POST">
    @csrf
    ...
    @else
    <p>Bạn cần phải <a href="{{ route('login.form') }}">đăng nhập</a> để phản hồi.</p>
    @endauth
</div>
