...
class Product extends Model
{
    ...
    public static function getStockByCategory($categoryId = null)
    {
        $query = Product::query();

        // Nếu có `categoryId`, lọc sản phẩm theo loại
        if ($categoryId) {
            $query->where('cat_id', $categoryId);
        }

        // Lấy thông tin tồn kho của từng sản phẩm và kèm theo thông tin loại sản phẩm (chỉ trường title)
        return $query->select('id', 'title', 'stock', 'cat_id')
