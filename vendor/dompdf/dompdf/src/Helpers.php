...
class Helpers
{
    ...
    {
        // Xử lý các loại cảnh báo được quan tâm
        if (!($errno & (E_WARNING | E_NOTICE | E_USER_NOTICE | E_USER_WARNING | E_DEPRECATED | E_USER_DEPRECATED))) {
            // Ghi nhận lỗi không quan trọng nhưng không ném ngoại lệ
            error_log("Dompdf Ignored Error: $errstr in $errfile on line $errline [$errno]");
            return true;
        }
        ...
        // Ghi nhận lỗi deprecated để tránh ngắt tiến trình
        if ($errno === E_DEPRECATED || $errno === E_USER_DEPRECATED) {
            error_log("Dompdf Deprecated Warning: $errstr in $errfile on line $errline [$errno]");
            $_dompdf_warnings[] = "Deprecated: $errstr in $errfile on line $errline";
            return true; // Chỉ ghi nhận cảnh báo
        }

        // Hiển thị cảnh báo nếu tùy chọn được bật
        if ($_dompdf_show_warnings) {
            echo "Warning: $errstr in $errfile on line $errline\n";
        }

        // Lưu cảnh báo vào mảng toàn cục
        $_dompdf_warnings[] = $errstr;

        return true; // Ngăn không ném lỗi mặc định ra ngoài
    }
